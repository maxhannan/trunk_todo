...
const sideBar = (() => {
  ...
  let addBtn, container, delBtns, filters
  const sections = JSON.parse(localStorage.getItem('sectARR')) || []
  const bindEvents = () => {
  ...
  const AddSection = () => {
    ...
    let validName = sectionName.length > 0
    while (!validName) {
      sectionName = prompt('Try Again')
      validName = sectionName.length > 1
    }
